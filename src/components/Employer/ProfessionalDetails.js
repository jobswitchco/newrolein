...
const [tabTimers, setTabTimers] = useState({ 0: 0, 1: 0, 2: 0 });
const [tabEnterTime, setTabEnterTime] = useState(Date.now());

const tabTimersRef = useRef(tabTimers);
...
const handleTabChange = (event, newValue) => {
  const now = Date.now();
  const duration = Math.floor((now - tabEnterTime) / 1000);

  setTabTimers(prev => ({
    ...prev,
    [activeTab]: prev[activeTab] + duration
  }));
...
const handleClick = async () => {
 ...
 const formatCTC = (amount, currency) => {
    ...
    useEffect(() => {
  tabTimersRef.current = tabTimers;
}, [tabTimers]);
...
useEffect(() => {
  tabEnterTimeRef.current = tabEnterTime;
}, [tabEnterTime]);
...
useEffect(() => {
  if (open && userId) {
    ...
    tabEnterTimeRef.current = now;

    axios.post(`${baseUrl}/track-profile-view`, { userId }, { withCredentials: true })
  ...
  return () => {
    ...
    const finalTab = activeTabRef.current;
    const finalTabDuration = Math.floor((endTime - tabEnterTimeRef.current) / 1000);

    const updatedTabTimers = {
      ...tabTimersRef.current,
      [finalTab]: (tabTimersRef.current[finalTab] || 0) + finalTabDuration
    };

    axios.post(`${baseUrl}/update-view-duration`, {
      userId,
      durations: updatedTabTimers,
      totalDuration
    }, { withCredentials: true })
    .catch(err => console.error("Duration update error:", err));
  };
}, [open, userId]);
